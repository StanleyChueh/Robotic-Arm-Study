...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def load_demo_info(self, filter_by_attribute=None, demos=None):
        ...
        for ep in self.demos:
            dataset_path = "data/{}".format(ep)
            print(f"🚀 DEBUG: Checking attributes for {dataset_path}: {list(self.hdf5_file[dataset_path].attrs.keys())}")

            if "num_samples" not in self.hdf5_file[dataset_path].attrs:
                raise KeyError(f"❌ ERROR: 'num_samples' not found in {dataset_path}. Found attributes: {list(self.hdf5_file[dataset_path].attrs.keys())}")

            demo_length = self.hdf5_file[dataset_path].attrs["num_samples"]
    ...
    def hdf5_file(self):
        ...
        if self._hdf5_file is None:
            self._hdf5_file = h5py.File(self.hdf5_path, 'r',libver='latest')
