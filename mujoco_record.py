...
env = suite.make(
    env_name="Lift",
    robots="Panda",
    use_camera_obs=False,  # Disable images for low-dim training
    controller_configs=suite.load_controller_config(default_controller="OSC_POSE"),
    has_renderer=True,  # Enables on-screen rendering
    render_camera="birdview",  # Specify a camera for rendering
)
...
shape_meta = {
    "all_shapes": {
        "robot0_proprio-state": [env.observation_spec()["robot0_proprio-state"].shape[0]],  # Dynamic proprio-state size
        "robot0_eef_pos": [3],
        "robot0_eef_quat": [4],
        ...
        "robot0_joint_vel": [7],
        "object": [10],
        "actions": [env.action_dim],  # Dynamically set action dimension
    },
    "use_images": False,
    "use_depths": False,
}
...
def get_observations(obs):
    """ Safely extracts available observations and ensures compatibility. """
    #available_obs = obs.keys()  # List of available observation keys

    return {
        "robot0_proprio-state": obs["robot0_proprio-state"],
        "robot0_eef_pos": obs.get("robot0_eef_pos", np.zeros(3)),  # Default to zero if missing
        ...
        "object": obs.get("object-state", np.zeros(10)),  # Adjust default shape if needed
    }
...
for ep in range(num_episodes):
    ...
    for key in obs_list[0].keys():
        episode_group.create_dataset(f"obs/{key}", data=np.array([o[key] for o in obs_list]))
